function r = solo_get_session_hr(x)
%
% Returns hit rate overall for session in x.
%
...
ntrials_s1 = length(find(trials(:,1)==1));

r = length(find(trials(:,1)==1 & trials(:,2)==1)) / ntrials_s1;    
