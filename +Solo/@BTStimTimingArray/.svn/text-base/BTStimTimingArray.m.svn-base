...
classdef BTStimTimingArray < handle
    ...
    properties (Dependent = true)
        ...
        trialTypes
        trialCorrects
        trialResponses % lick = 1, nolick = 0
        trimmedTrialNums
        fractionCorrect
        volumeH20Consumed
        EmbCLog
    ...
    methods (Access = public)
        function obj = BTStimTimingArray(x, session_name)
            ...
            for k=1:n_trials
                ...
                response = x.saved.stim_timingobj_response_history(k);
                ...
                if ismember(session_type, {'Discrim'})  % For now limit only to Discrim trials
                    behav_trial = Solo.BTStimTiming(mouse_name, session_name, trial_num, trial_type,...
                        ...
                        'nogoPosition', nogo_position, 'goPosition', go_position);
        ...
        function r = trialTypePerformanceMeans(obj)
            tt = obj.trialTypes(obj.trim(1):(end-obj.trim(2)));
            correct = obj.trialCorrects(obj.trim(1):(end-obj.trim(2)));
            ...
            nboot = 1000;
            go = correct(strcmp('Go',tt)); go_se = std(bootstrp(nboot,@mean, go));
            ng = correct(strcmp('Nogo',tt)); ng_se = std(bootstrp(nboot,@mean, ng));
            go0 = correct(strcmp('GoStim_0ms',tt)); go0_se = std(bootstrp(nboot,@mean, go0));
            go5 = correct(strcmp('GoStim_5ms',tt)); go5_se = std(bootstrp(nboot,@mean, go5));
            go10 = correct(strcmp('GoStim_10ms',tt)); go10_se = std(bootstrp(nboot,@mean, go10));
            go20 = correct(strcmp('GoStim_20ms',tt)); go20_se = std(bootstrp(nboot,@mean, go20));
            go50 = correct(strcmp('GoStim_50ms',tt)); go50_se = std(bootstrp(nboot,@mean, go50));
            ng0 = correct(strcmp('NogoStim_0ms',tt)); ng0_se = std(bootstrp(nboot,@mean, ng0));
            ng5 = correct(strcmp('NogoStim_5ms',tt)); ng5_se = std(bootstrp(nboot,@mean, ng5));
            ng10 = correct(strcmp('NogoStim_10ms',tt)); ng10_se = std(bootstrp(nboot,@mean, ng10));
            ng20 = correct(strcmp('NogoStim_20ms',tt)); ng20_se = std(bootstrp(nboot,@mean, ng20));
            ng50 = correct(strcmp('NogoStim_50ms',tt)); ng50_se = std(bootstrp(nboot,@mean, ng50));
            
            
            y_go = [mean(go0) mean(go5) mean(go10) mean(go20) mean(go50)];
            y_go_se = [go0_se go5_se go10_se go20_se go50_se];
            y_ng = [mean(ng0) mean(ng5) mean(ng10) mean(ng20) mean(ng50)];
            y_ng_se = [ng0_se ng5_se ng10_se ng20_se ng50_se];
            ...
            r.goStimSE = y_go_se;
            r.nogoStim = y_ng;
            r.nogoStimSE = y_ng_se;
            r.stimDelay = delay;
            r.goNoStim = mean(go);
            r.goNoStimSE = go_se;
            r.nogoNoStim = mean(ng);
            r.nogoNoStimSE = ng_se;
        end
...
%             elseif strcmp(tt,'VirtUnpunishedNogo')
        ...
        function value = get.trialTypes(obj)
            if ~isempty(obj.trials)
                value = cellfun(@(x) x.trialType, obj.trials,'UniformOutput',false);
            else
                value = [];
            end
        end
        
        function value = get.EmbCLog(obj)
            if ~isempty(obj.trials)
                value = cellfun(@(x) x.trialEmbCLogItems, obj.trials,'UniformOutput',false);
            else
                value = [];
            end
        end
        ...
        function value = get.stimGiven(obj)
            if ~isempty(obj.trials)
                value = cellfun(@(x) x.stimGiven, obj.trials);
            else
                value = [];
            end
        end
        ...
        function value = get.numStims(obj)
            if ~isempty(obj.trials)
                value = cellfun(@(x) x.numStims, obj.trials);
            else
                value = [];
            end
        end
        ...
        function value = get.trialResponses(obj)
            if ~isempty(obj.trials)
                value = cellfun(@(x) x.response, obj.trials);
            else
                value = [];
            end
        end
