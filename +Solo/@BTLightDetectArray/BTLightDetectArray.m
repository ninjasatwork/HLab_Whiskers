...
classdef BTLightDetectArray < handle
    ...
    methods (Access = public)
        function obj = BTLightDetectArray(x, session_name)
            %
            % function obj = BTLightDetectArray(x, session_name)
            %
            ...
            for k=1:n_trials
                ...
                response = x.saved.light_detectobj_response_history(k);
                ...
                if ismember(session_type, {'Discrim'})  % For now limit only to Discrim trials
                    behav_trial = Solo.BTLightDetect(mouse_name, session_name, trial_num, trial_type,...
                        response, trial_events, next_trial_events, 'useFlag', use_flag, 'sessionType',...
                        session_type, 'extraITIOnErrorSetting', extra_ITI_on_error, 'samplingPeriodTimeSetting', sampling_period_time, ...
                        'waterValveTimeSetting', water_valve_time);
